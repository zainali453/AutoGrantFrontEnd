...
const CrawlingDashboard = () => {
  ...
  const startCrawling = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/admin/crawler/crawlwebsites', {
        headers: {
  ...
  return (
    <div className="container mx-auto pt-20 pl-96 pr-56 bg-gray-500">
...
const getWebsitesData = async (setEntries) => {
  try {
    ...
    const response = await axios.get('http://localhost:4000/admin/getwebsitesdata?type=all', {
      headers: {
        'x-access-token': token,
